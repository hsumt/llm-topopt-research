...
from project.formulation.critic_view import compact_provenance
...
def _render_visual_evidence_strip(attachments: list[ContextAttachment], *, max_items: int = 4) -> None:
    """Show a compact visual mirror without requiring the Visuals tab."""
    ...
    for attachment in attachments:
        ...
        media = attachment.media_type.lower()
        if media.startswith("image/"):
            with columns[rendered % 2]:
                st.image(attachment.data, caption=attachment.name, use_container_width=True)
            rendered += 1
        elif media == "application/pdf":
            for label, png in render_pdf_pages(attachment, max_pages=min(3, max_items - rendered)):
                with columns[rendered % 2]:
                    st.image(png, caption=label, use_container_width=True)
                rendered += 1
                if rendered >= max_items:
                    break
    if rendered == 0 and attachments:
        st.caption("Visual files are available in the Visuals tab, but no local thumbnail could be rendered.")


def _render_related_context(session, context_ids: list[str]) -> None:
    if not context_ids:
        return
    by_id = {item.id: item for item in session.retrieved_context}
    candidate_by_id = {item.id: item for item in session.parser_result.context_candidates}
    with st.expander("Show evidence used for this question"):
        found = False
        for cid in context_ids:
            if cid in by_id:
                item = by_id[cid]
                st.markdown(f"**{cid} · {item.source}**")
                st.write(item.text)
                found = True
            elif cid in candidate_by_id:
                item = candidate_by_id[cid]
                st.markdown(f"**{cid} · context candidate**")
                st.write(item.text)
                st.caption(item.reason)
                found = True
        if not found:
            st.caption("The referenced context id is retained in the session but is not in the latest retrieval window.")
        if attachments := st.session_state.get("attachments", []):
            st.caption("Relevant drawings/PDFs remain visible in the Visuals tab and the visual mirror on the Problem tab.")
...
if session is None:
    ...
    if submitted:
        if not problem.strip():
            ...
            # Debug artifacts represent only the CURRENT formulation attempt.
            # Remove stale parser/critic snapshots before starting so an unrelated
            # downstream failure cannot be mislabeled in the dashboard.
            parser_debug_path = ARTIFACT_ROOT / "debug" / "parser_last_failure.json"
            critic_debug_path = ARTIFACT_ROOT / "debug" / "critic_last_failure.json"
            for debug_path in (parser_debug_path, critic_debug_path):
                try:
            ...
            with st.status("Starting formulation...", expanded=True) as status:
                ...
                try:
                    ...
                    if critic_debug_path.exists():
                        st.warning(
                            "A fresh formulation-critic failure snapshot was generated by THIS attempt. "
                            "Download it before running the model again; the cached/raw critic output "
                            "can be debugged without paying for another critic call."
                        )
                        st.download_button(
                            "Download critic failure snapshot from this attempt",
                            data=critic_debug_path.read_bytes(),
                            file_name="critic_last_failure.json",
                            mime="application/json",
                            key="download_critic_failure",
                        )
                    elif parser_debug_path.exists():
                        st.warning(
                            "A fresh parser failure snapshot was generated by THIS attempt. "
                            "Download it before running the model again so the next fix can "
                            "be made without another paid parser call."
                        )
                        st.download_button(
                            "Download parser failure snapshot from this attempt",
                            data=parser_debug_path.read_bytes(),
                        ...
                        st.info(
                            "No parser or critic failure snapshot was generated by this attempt. "
                            "The failure occurred in another deterministic stage. Copy the red error message above."
                        )
        ...
        with st.expander("LLM usage"):
            ...
            for item in session.usage:
                rows.append({
                    ...
                    "input": int(item.get("input_tokens", 0)),
                    "output": int(item.get("output_tokens", 0)),
                    "total": int(item.get("total_tokens", 0)),
                })
            st.dataframe(rows, hide_index=True, use_container_width=True)
            with st.expander("Detailed usage diagnostics"):
        ...
        with st.form(f"clarification_packet_{session.revision}"):
            ...
            for index, question in enumerate(packet, start=1):
                ...
                if question.related_context_ids:
                    ...
                    _render_related_context(session, question.related_context_ids)
            ...
            if not any(value.strip() for value in answers.values()):
                ...
                with st.status("Applying clarification packet...", expanded=True) as status:
                    ...
                    try:
                        ...
                        resolver_debug_path = ARTIFACT_ROOT / "debug" / "resolver_last_failure.json"
                        if resolver_debug_path.exists():
                            st.warning(
                                "The resolver response was saved locally. Download/replay it before "
                                "calling the model again; this failure can be debugged offline."
                            )
                            st.download_button(
                                "Download resolver failure snapshot",
                                data=resolver_debug_path.read_bytes(),
                                file_name="resolver_last_failure.json",
                                mime="application/json",
                                key="download_resolver_failure_packet",
                            )
                st.rerun()
                ...
                try:
                    ...
                    if resolver_debug_path.exists():
                        ...
                        st.download_button(
                            ...
                            file_name="resolver_last_failure.json",
                            mime="application/json",
                            key="download_resolver_failure_chat",
                        )
            st.rerun()
...
with mirror_col:
    ...
    with overview_tab:
        ...
        if attachments:
            ...
            st.caption(
                "User-supplied evidence shown alongside the evolving text formulation. "
                "These images are evidence, not AI-generated geometry."
            )
            _render_visual_evidence_strip(attachments, max_items=4)
    ...
    with prov_tab:
        if not session.parser_result.field_provenance:
            ...
            st.caption(
                "Compact provenance is shown by default. The deterministic leaf-level audit "
                "is still retained for export/debugging."
            )
            compact = compact_provenance(session.parser_result)
            for item in compact:
                with st.expander(
                    f"{item['scope']} · {item['source']} · {item['leaf_count']} audited field(s)"
                ):
                    st.write(f"**Evidence:** {item['evidence']}")
                    st.write(f"**Confidence:** {item['confidence']:.2f}")
                    st.write("**Example audited paths:**")
                    for path in item["sample_paths"]:
                        st.code(path, language=None)

            if st.checkbox("Show leaf-level provenance audit", value=False):
